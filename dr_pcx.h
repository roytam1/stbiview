...
drpcx_uint8* drpcx_load_file(const char* filename, drpcx_bool32 flipped, int* x, int* y, int* internalComponents, int desiredComponents)
{
    ...
    pImageData = drpcx_load(drpcx__on_read_stdio, pFile, flipped, x, y, internalComponents, desiredComponents);
...
drpcx_bool32 drpcx__decode_2bit(drpcx* pPCX)
{
    ...
    switch (pPCX->header.bitPlanes)
    {
        case 1:
        {
            ...
            cgaBGColor   = pPCX->header.palette16[0] >> 4;
            i = (pPCX->header.palette16[3] & 0x20) >> 5;
            p = (pPCX->header.palette16[3] & 0x40) >> 6;
            //c = (pPCX->header.palette16[3] & 0x80) >> 7;    // Color or monochrome. How is monochrome handled?
...
drpcx_bool32 drpcx__decode_8bit(drpcx* pPCX)
{
    ...
    switch (pPCX->header.bitPlanes)
    {
        case 1:
        {
            ...
            paletteMarker = drpcx__read_byte(pPCX);
            if (paletteMarker == 0x0C) {
...
drpcx_uint8* drpcx_load(drpcx_read_proc onRead, void* pUserData, drpcx_bool32 flipped, int* x, int* y, int* internalComponents, int desiredComponents)
{
    ...
    dataSize = pcx.width * pcx.height * pcx.components;
    ...
    if (desiredComponents != (int)pcx.components) {
        drpcx_uint8 *pSrcData, *pDstData, *pNewImageData = (drpcx_uint8*)malloc(pcx.width * pcx.height * desiredComponents);
        if (pNewImageData == NULL) {
            free(pcx.pImageData);
            return NULL;
        }

        pSrcData = pcx.pImageData;
        pDstData = pNewImageData;
        if (desiredComponents < (int)pcx.components) {
