...
{0x00,0xDF,0xAA},{0x00,0xDF,0xFF},{0x00,0xFF,0x55},{0x00,0xFF,0xAA},{0xD6,0x00,0x93},{0x77,0x77,0x77},{0x86,0x86,0x86},{0x96,0x96,0x96},
{0xB2,0xB2,0xB2},{0xA5,0x00,0x21},{0x2A,0x1F,0xAA},{0x2A,0x1F,0xFF},{0xFF,0x7C,0x80},{0x2A,0x3F,0x55},{0xFF,0xFB,0xF0},{0x3A,0x6E,0xA5},
...
void LoadImageFromPath(HWND hwnd, char* filePath) {
    ...
    if(fileExt && stricmp(fileExt,".p2") == 0) {
        isWebp = 1; // not really webp, but same malloc style as webp
        pSrc = LoadPIC2(filePath, &imgW, &imgH);
    }
    else
    if(fileExt && stricmp(fileExt,".y4m") == 0) {
        isWebp = 1; // not really webp, but same malloc style as webp
        pSrc = LoadY4M(filePath, &imgW, &imgH);
    }
    else
